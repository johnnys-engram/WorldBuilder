...
public static class DatIconLoader {
    public static WriteableBitmap? LoadIcon(IDatReaderWriter dats, uint iconId, int size = 32) {
        try {
            ...
            dats.HighRes.TryGet<RenderSurface>(iconId, out var rsHigh);
            dats.Portal.TryGet<RenderSurface>(iconId, out var rsPortal);
            var rs = rsHigh ?? rsPortal;
    ...
    public static WriteableBitmap? LoadSurfaceIcon(IDatReaderWriter dats, uint surfaceId, int size = 32) {
        try {
            ...
            dats.HighRes.TryGet<Surface>(surfaceId, out var surfaceHigh);
            dats.Portal.TryGet<Surface>(surfaceId, out var surfacePortal);
            var surface = surfaceHigh ?? surfacePortal;
            ...
            dats.HighRes.TryGet<SurfaceTexture>(surface.OrigTextureId, out var texHigh);
            dats.Portal.TryGet<SurfaceTexture>(surface.OrigTextureId, out var texPortal);
            var surfTex = texHigh ?? texPortal;
            if (surfTex is null || surfTex.Textures == null || surfTex.Textures.Count == 0) return null;

            var rsId = surfTex.Textures[surfTex.Textures.Count - 1];
            dats.HighRes.TryGet<RenderSurface>(rsId, out var rsHigh);
            dats.Portal.TryGet<RenderSurface>(rsId, out var rsPortal);
            var rs = rsHigh ?? rsPortal;
            if (rs is null) return null;
            return RenderSurfaceToBitmap(rs, size);
        }
